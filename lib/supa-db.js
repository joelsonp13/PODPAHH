...
async function requestPasswordReset(email) {
  ...
  const item = {
    ...
    token,
    expires_at: new Date(Date.now() + RESET_TTL).toISOString(),
    used: false
  };
  ...
  if (!sent.success) {
    await sb().from('password_resets').delete().eq('id', item.id);
...
async function resetPasswordByToken(token, newPass) {
  ...
  const found = await sb().from('password_resets').select('*')
    .eq('token', String(token || '')).eq('used', false)
  ...
  const newToken = await createCustomerSession(row.customer_id);
  return { success: true, token: newToken, data: safeCustomer(upd.data) };
}

// Compat: assinatura antiga (email, code, newPass) — agora usa token.
...
async function listResets() {
  ...
  for (const r of (data || [])) {
    ...
    out.push({ id: r.id, email: r.email, customer_name: cust.name || '', customer_phone: cust.phone || '', expires_at: r.expires_at, created_at: r.created_at });
  }
  return out;
}
...
module.exports = {
  ...
  requestPasswordReset, resetPassword, resetPasswordByToken, listResets, revokeReset,
