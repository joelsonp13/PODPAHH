...
async function registerCustomer(name, email, password, phone) {
  ...
  await ensureAuthUser(safeCustomer(data), password);
...
/* ---------------- PASSWORD RESET (e-mail do Supabase Auth) ----------------
   Espelha requestPasswordReset/resetPasswordSupabase do backend local. */
async function ensureAuthUser(customer, rawPassword) {
  try {
    const { error } = await sb().auth.admin.createUser({
      email: customer.email,
      password: rawPassword ? String(rawPassword) : crypto.randomBytes(24).toString('hex'),
      email_confirm: true,
  ...
  } catch (e) { /* best-effort: nunca quebra o fluxo */ }
}
...
async function requestPasswordReset(email) {
  ...
  const cust = await sb().from('customers').select('id,email,name').eq('email', mail).limit(1);
  if (cust.error) return { success: false, status: 500, error: cust.error.message };
  if (!cust.data.length) return { success: true, data: { emailed: false } };
  try {
    await ensureAuthUser(cust.data[0], null);
    const { error } = await anonClient().auth.resetPasswordForEmail(cust.data[0].email, {
    ...
    return { success: false, status: 502, error: 'Provedor de e-mail indisponível.' };
  }
}
...
async function resetPasswordSupabase(supaToken, newPass) {
  ...
  let authUser;
  try {
    const { data, error } = await sb().auth.getUser(String(supaToken));
  ...
  const found = await sb().from('customers').select('*').eq('email', mail).limit(1);
  if (found.error) return { success: false, status: 500, error: found.error.message };
  if (!found.data.length) return { success: false, status: 404, error: 'Conta não encontrada.' };
  const customer = found.data[0];
  const upd = await sb().from('customers').update({ password: hashPassword(newPass) }).eq('id', customer.id).select().single();
  if (upd.error) return { success: false, status: 500, error: upd.error.message };
  try {
    await sb().auth.admin.updateUserById(authUser.id, { password: String(newPass) });
  } catch (e) { /* hash local é a fonte da verdade */ }
  await sb().from('customer_sessions').delete().eq('customer_id', customer.id);
  const token = await createCustomerSession(customer.id);
  return { success: true, token, data: safeCustomer(upd.data) };
}

// Compat antiga: removida — use resetPasswordSupabase.
async function resetPassword() {
  return { success: false, status: 410, error: 'Fluxo antigo desativado. Use o link do e-mail.' };
}

async function listResets() { return []; }
async function revokeReset() {
  return { success: false, status: 404, error: 'Recurso desativado.' };
}
...
/* ---------------- STORAGE (bucket "imagens", público) ----------------
   dataURL -> upload -> URL pública. Fora dataURL, passa direto.
   Falha no upload = mantém dataURL (nunca perde a imagem). */
async function uploadDataUrl(dataUrl, dest) {
  const m = String(dataUrl || '').match(/^data:(image\/[a-zA-Z0-9.+-]+);base64,([\s\S]+)$/);
  if (!m) return null;
  let buf;
  try { buf = Buffer.from(m[2], 'base64'); } catch (e) { return null; }
  if (!buf.length || buf.length > 2500000) return null;
  const ext = m[1] === 'image/png' ? 'png' : (m[1] === 'image/webp' ? 'webp' : 'jpg');
  const path = dest + '.' + ext;
  const { error } = await sb().storage.from('imagens').upload(path, buf, { contentType: m[1], upsert: true });
...
async function storeImage(v, dest) {
  ...
  if (v.indexOf('data:') !== 0) return v;
  try {
    return (await uploadDataUrl(v, dest)) || v;
  } catch (e) { return v; }
}
...
async function saveProduct(productData) {
  ...
  if (pd.id) {
    ...
    if (cur.data.length) {
      ...
      // dataURL -> Storage (mantém URL existente se não mudou)
      const img = (pd.image && pd.image !== prev.image)
        ? await storeImage(sanitizeImageUrl(pd.image), 'products/' + pd.id + '/cover')
        : (prev.image || '');
      for (const m of models) {
        if (m.image && m.image.indexOf('data:') === 0) {
          m.image = await storeImage(m.image, 'products/' + pd.id + '/' + m.id);
        }
      }
      const row = {
  ...
  const img = await storeImage(sanitizeImageUrl(pd.image), 'products/' + newId + '/cover');
  for (const m of models) {
    if (m.image && m.image.indexOf('data:') === 0) {
      m.image = await storeImage(m.image, 'products/' + newId + '/' + m.id);
    }
  }
  const row = {
    id: newId,
...
module.exports = {
  ...
  requestPasswordReset, resetPassword, resetPasswordSupabase, listResets, revokeReset,
