...
async function pruneResets() {
  await sb().from('password_resets').delete().or(`used.eq.true,expires_at.lt.${new Date().toISOString()}`);
}
...
async function requestPasswordReset(email) {
  ...
  await pruneResets();
  const cust = await sb().from('customers').select('id,email').eq('email', mail).limit(1);
  if (cust.error) return { success: false, status: 500, error: cust.error.message };
  if (!cust.data.length) return { success: true, data: null };
  const customer = cust.data[0];
  const recent = await sb().from('password_resets').select('id,expires_at')
    .eq('customer_id', customer.id).eq('used', false).gt('expires_at', new Date().toISOString())
    .order('created_at', { ascending: false }).limit(1);
  if (!recent.error && recent.data.length) {
    return { success: true, data: { protocol: recent.data[0].id, expires_in: Math.max(1, Math.round((Date.parse(recent.data[0].expires_at) - Date.now()) / 1000)) } };
  }
  const now = Date.now();
  const item = {
    ...
    expires_at: new Date(now + RESET_TTL).toISOString(),
    used: false
  };
  await sb().from('password_resets').delete().eq('customer_id', customer.id);
  const ins = await sb().from('password_resets').insert(item);
...
async function resetPassword(email, code, newPass) {
  const mail = String(email || '').trim().toLowerCase();
  if (!newPass || String(newPass).length < 8) {
    return { success: false, status: 400, error: 'A nova senha precisa ter no mínimo 8 caracteres.' };
  }
  await pruneResets();
  const found = await sb().from('password_resets').select('*')
    .eq('email', mail).eq('code', String(code || '').trim()).eq('used', false)
  ...
  const upd = await sb().from('customers').update({ password: hashPassword(newPass) }).eq('id', row.customer_id).select().single();
  if (upd.error) return { success: false, status: 500, error: upd.error.message };
  await sb().from('password_resets').update({ used: true }).eq('id', row.id);
  await sb().from('password_resets').delete().eq('customer_id', row.customer_id);
  await sb().from('customer_sessions').delete().eq('customer_id', row.customer_id);
  const token = await createCustomerSession(row.customer_id);
  return { success: true, token, data: safeCustomer(upd.data) };
}

async function listResets() {
  await pruneResets();
  const { data, error } = await sb().from('password_resets').select('*').eq('used', false).order('created_at', { ascending: false });
  if (error) throw error;
  const out = [];
  for (const r of (data || [])) {
    const c = await sb().from('customers').select('name,phone').eq('id', r.customer_id).limit(1);
    const cust = (c.data && c.data[0]) || {};
    out.push({ id: r.id, email: r.email, code: r.code, customer_name: cust.name || '', customer_phone: cust.phone || '', expires_at: r.expires_at, created_at: r.created_at });
  }
  return out;
}

async function revokeReset(id) {
  const del = await sb().from('password_resets').delete().eq('id', String(id));
...
module.exports = {
  ...
  requestPasswordReset, resetPassword, listResets, revokeReset,
